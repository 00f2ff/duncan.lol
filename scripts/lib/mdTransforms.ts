...
function poetryPost(blocks: MdBlock[]): MdBlock[] {
  // fixme: clean up this code by flipping conditions
...
export async function transformMarkdown(
  ...
  pageData: PageDatum[],
): Promise<QualifiedPage> {
  const qualifiedPageWithBlockTransforms: QualifiedPage = await blocks.reduce(
    async (accPromise, block, index) => {
      ...
      switch (block.type) {
        ...
        case "image":
          const [relativeLinkBlock, imageBuffer] = await imageBlock(
            block,
            index,
            slug,
          );
