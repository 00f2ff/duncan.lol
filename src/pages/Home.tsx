...
export const Home = () => {
  ...
  const isEtAlia = (p: BlogPost): boolean => !p.tags.some(tag => ['Work', 'Stories', 'Poetry'].includes(tag))
  ...
  useEffect(() => {
    setTopics(topics => new Map(Array.from(topics.entries(), ([topic, ]) => [topic, posts.filter(p => topic == 'Et alia' ? isEtAlia(p) : p.tags.includes(topic)).length])))
  }, [posts])
  ...
  return (
    <Layout key="index">
      <div className="mb-10">
        ...
        {/* <CaptionBar /> */}
        <Topics names={Array.from(topics.entries(), ([k, v]) => `${k} (${v})`)} onClick={selectTopic} />
        {/* <Socials /> */}
      </div>
     
      {currentPosts && currentPosts.map((post) => <PostBlock key={post.slug} {...post} />)}
    </Layout>
  );
};